...
Morpho on Base uses official GraphQL queries for listed Vaults and Morpho Blue markets. Vault rows are lending-only; market rows are classified by whether canonical Base USDC is the loan asset (lend USDC) or collateral asset (borrow another asset against USDC). The product filter defaults to All, and borrow APY remains visibly distinct from lending yield. Live counts are snapshots, not hard-coded expectations.
